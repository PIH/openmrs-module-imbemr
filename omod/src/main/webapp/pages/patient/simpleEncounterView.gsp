<%
    ...
    def returnUrl = ui.pageLink("imbemr", "patient/encounterList", [ "patientId": encounter.patient.uuid ])
%>
...
<fieldset id="obs-details">
    ...
    <% encounter.getObsAtTopLevel(false).each{obs -> %>
        <div class="top-level-obs-section row col-12">
            ...
            <div class="value col-8">${ui.includeFragment("imbemr", "patient/obsValue", ["obs": obs, "padding": 0])}</div>
        </div>
    <% } %>
</fieldset>
...
<div>
    <input id="return-button" type="button" value="${ ui.message("imbemr.encounterList.return") }" onclick="location.href='${returnUrl}'" />
</div>
