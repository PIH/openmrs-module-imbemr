 ...
 */
package org.openmrs.module.imbemr.web;
...
public class TriggerSyncController {
    ...
    @Autowired
    ImbEmrService imbEmrService;

    @RequestMapping(value = "/module/imbemr/triggerSync.form")
    public void triggerSyncList(HttpServletResponse response,
        ...
        if (StringUtils.isNotBlank(patientId)) {
            ...
            try {
                List<String> log = imbEmrService.triggerSyncForPatient(patient);
                messages.addAll(log);
            }
            catch (Exception e) {
                data.put("error", e.getMessage());
